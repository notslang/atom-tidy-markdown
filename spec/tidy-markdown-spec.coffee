...
describe "Tidy Markdown", ->
  ...
  beforeEach ->
    ...
    filePath = path.join(directory, 'atom-tidy-markdown.txt')
    ...
    waitsForPromise ->
      atom.packages.activatePackage('tidy-markdown')
  ...
  describe "when the editor is destroyed", ->
    ...
    it "does not leak subscriptions", ->
      {whitespace} = atom.packages.getActivePackage('tidy-markdown').mainModule
      expect(whitespace.subscriptions.disposables.size).toBe 2

      atom.packages.deactivatePackage('tidy-markdown')
  ...
  describe "when deactivated", ->
    it "does not tidy markdown in editors opened after deactivation", ->
      atom.config.set("whitespace.removeTrailingWhitespace", true)
      atom.packages.deactivatePackage('tidy-markdown')
      text = "# foo #\n"
      editor.setText text
  ...
  describe "when the 'tidy-markdown:run' command is run", ->
    it "cleans up the Markdown", ->
      buffer.setText "# foo #\n"
      atom.commands.dispatch(workspaceElement, 'tidy-markdown:run')
      expect(buffer.getText()).toBe "# foo\n"
